...
async function Page({ params }: { params: { slug: string , spaceName: string } }) {
  ...
  return (
    <section className="w-full ">
      <div className="flex justify-start">
        <Link
          href={`/spaces/${params.spaceName}/blogs/${params.slug}`}
        ...
        <div className="max-w-4xl mx-auto flex flex-col w-full gap-4 p-8 bg-secondary-100 dark:bg-secondary-900 rounded">
          <div className="min-w-[20rem] max-w-4xl mx-auto flex flex-col w-full gap-4">
            ...
            <Suspense fallback={<div>Loading..</div>}>
              <UpdateBlogForm categories={categories} blog={blog} files={files} spaceName={params.spaceName} />
            </Suspense>
          </div>
        </div>
      </div>
